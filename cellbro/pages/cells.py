...
from cellbro.plots.Heatmap import Heatmap
from cellbro.plots.Projection import PCA, TSNE, UMAP, Projection, Trimap
...
def create_page(dash_app, dataset):
    ...
    def _(submit, projection_color, projection_type, **kwargs):
        if projection_type == "UMAP":
            projection_params = dict(
                [
                    (key.replace("umap_", ""), kwargs[key])
                    for key in kwargs.keys()
                    if key.startswith("umap_")
                ]
            )
            return (
                UMAP(dataset, projection_color, projection_params).plot(),
                {"display": "block"},
                {"display": "none"},
                {"display": "none"},
                {"display": "none"},
            ...
            projection_params = dict(
                [
                    (key.replace("tsne_", ""), kwargs[key])
                    for key in kwargs.keys()
                    if key.startswith("tsne_")
                ]
            )
            return (
                TSNE(dataset, projection_color, projection_params).plot(),
                {"display": "none"},
                {"display": "block"},
                {"display": "none"},
                {"display": "none"},
            ...
            projection_params = dict(
                [
                    (key.replace("trimap_", ""), kwargs[key])
                    for key in kwargs.keys()
                    if key.startswith("trimap_")
                ]
            )
            return (
                Trimap(dataset, projection_color, projection_params).plot(),
                {"display": "none"},
                {"display": "none"},
                {"display": "block"},
                {"display": "none"},
            ...
            projection_params = dict(
                [
                    (key.replace("pca_", ""), kwargs[key])
                    for key in kwargs.keys()
                    if key.startswith("pca_")
                ]
            )
            return (
                PCA(dataset, projection_color, projection_params).plot(),
                {"display": "none"},
                {"display": "none"},
                {"display": "none"},
                {"display": "block"},
    ...
    # Violin
    @dash_app.callback(
        output=Violin.get_callback_outputs(), inputs=Violin.get_callback_inputs()
    )
