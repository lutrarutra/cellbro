...
def get(id: int) -> sa.Select[tuple[Variable]]:
    q =  sa.select(Variable)
    q = q.where(Variable.id == id)
    return q

def get_by_name_and_layer(name: str, layer: AnnDataLayerType) -> sa.Select[tuple[Variable]]:
    q =  sa.select(Variable)
    q = q.where(Variable.name == name)
    q = q.where(Variable.layer == layer)
    return q

def create(name: str, layer: AnnDataLayerType, type: VariableType) -> Variable:
